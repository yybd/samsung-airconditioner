...
SamsungDiscovery.prototype.listen = function(ifname, ipaddr, portno) {
  ...
  .on('response', function(msg, rinfo) {
    ...
    if(info.MODELCODE == 'SAMSUNG_DEVICE'){
      if(!self.devices.hasOwnProperty(mac)){
        ...
        self.emit('discover device', self.devices[mac]);
      }
      if(self.devices.hasOwnProperty(mac) && self.devices[mac].ip != rinfo.address){
        /*
        delete self.devices[mac];
        self.devices[mac] = new Device({
          logger : self.logger,
          ip     : rinfo.address,
          duid   : mac,
        ...
        */
        self.devices[mac].ip = rinfo.address;
        self.devices[mac].options.ip = rinfo.address;
        self.emit('change ip', self.devices[mac]);
      }
  ...
  ssdp.sock.on('listening', function() {
    setInterval(notify, 600 * 1000);
    notify();
    self.on('notify' , function(){notify(); console.log('on notify')});
  });
};
/*
SamsungDiscovery.prototype.notify = function(){
  var notify = function() {
    ssdp.notify(ifname, ipaddr, portno, 'AIR CONDITIONER',
                { SPEC_VER: 'MSpec-1.00', SERVICE_NAME: 'ControlServer-MLib', MESSAGE_TYPE: 'CONTROLLER_START' });
  };
